...
glm::vec3 Renderer::computePhongShading(const glm::vec3& color, const volume::GradientVoxel& gradient, const glm::vec3& L, const glm::vec3& V)
{
	...
	return ka * color + kd * glm::abs(glm::dot(L_norm, N_norm)) * color + ks * pow(glm::max(glm::dot(R_norm, V_norm), 0.0f), a) * color; //we use the absolute value to make the composite images brighter.
	//return glm::vec3(0.0f);
}
